...
**Progress (as of commit `1008dfe`):** **Phases 1–5 complete (Tasks 1–59). Ready to ship.** Foundation, manifest, step (all 8 types: shell, homebrew_bootstrap, brew_package, brew_cask, brewfile, git_clone, npm_global, git_config), tool, runner, linker, CLI, and TUI layers in place. `dot list`, `dot install`, `dot update`, `dot deploy`, `dot status`, `dot version` all wired. Default `dot` invocation launches the Bubble Tea TUI (picker + runner pane + conflict modal). Linker handles symlinks with conflict resolution (`--on-conflict=backup|overwrite|skip|abort`). Per-run log files at `~/.local/state/dot/runs/`. Formatted end-of-run summary with retry hints. TUI snapshot baseline established (`make snapshot` / `make test-snapshot`). All 13 tool manifests embedded (homebrew, rust, oh-my-zsh, nvm, claude-code, git-hooks, git, zsh, vim, ghostty, k9s, mise, claude). Legacy bash scripts removed; README rewritten around `dot`. Goreleaser config, release workflow, macOS integration workflow, and curl bootstrap installer all in place. `go test ./...` green across 12 packages. **Next up: tag `v0.0.1-rc1` and verify the curl one-liner on a fresh machine.**
